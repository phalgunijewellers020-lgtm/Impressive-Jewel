...
const COLUMNS = [
  ...
  { key: "weight", label: "Return Weight (g)", render: (r: Record<string, unknown>) => <span className="font-mono-data text-sm">{Number(r.weight).toFixed(3)}</span> },
...
export default function PolishReturn() {
  return (
    <TransactionPage
      title="Polish Return"
      subtitle="Record silver returned after polishing"
      table="polish_return"
      selectQuery="*, employee:employees(name,employee_code), item:items(name)"
      columns={COLUMNS}
      fields={FIELDS}
      emptyMessage="No Polish Return records."
      emptyIcon="↓"
      module="polish"
    />
  );
